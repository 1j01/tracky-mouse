...
function get_scoreboard_slot() {
	// TODO: differentiate pen input from mouse input... for both dwell clicking and manual clicking?
	if (round.used_keyboard) {
		...
		// next easiest is clicking with a mouse
		return "with_mouse";
	} else if (round.used_manual_movement) {
		// next easiest is moving the mouse but using the dwell clicker
		...
		// }
		// Of course, that's the most like cheating of all, so it feels like it should be first,
	...
	if (document.querySelectorAll(".archery-target:not(.hit)").length === 0) {
		...
		for (const [id, time] of Object.entries(best_times)) {
			...
			const label = {
				...
				with_dwell_clicker: "With Dwell Clicking",
				with_mouse: "With Mouse Clicking",
