						...
						if (repoRoot) {
							...
							try {
								...
								const { response: restartChoice } = await dialog.showMessageBox({
									...
									buttons: [t("desktop.updater.action.restartNow", { defaultValue: 'Restart Now' }), t("common.later", { defaultValue: 'Later' })],
									defaultId: 0,
									cancelId: 1
								});
								if (restartChoice === 0) {
								...
								const friendlyMessage = {
									...
									install: t("desktop.updater.errors.installDependencies", { defaultValue: "Failed to install dependencies for the new version after checking it out from git." })
